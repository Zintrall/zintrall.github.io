...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('use-custom-date').addEventListener('change', function() {
        document.getElementById('custom-date-filter').style.display = this.checked ? 'block' : 'none';
        document.getElementById('patches-filter').style.display = this.checked ? 'none' : 'block';
    });
    
    // Show more/less player rankings
    document.getElementById('show-more-winrate').addEventListener('click', function() {
        showMoreRows('player-winrate-table');
        this.style.display = 'none';
        document.getElementById('show-less-winrate').style.display = 'inline-block';
    });
    
    document.getElementById('show-less-winrate').addEventListener('click', function() {
        showLessRows('player-winrate-table');
        this.style.display = 'none';
        document.getElementById('show-more-winrate').style.display = 'inline-block';
    });
    
    document.getElementById('show-more-games').addEventListener('click', function() {
        showMoreRows('player-games-table');
        this.style.display = 'none';
        document.getElementById('show-less-games').style.display = 'inline-block';
    });
    
    document.getElementById('show-less-games').addEventListener('click', function() {
    ...
    document.getElementById('save-library').addEventListener('click', saveLibraryToFile);
    document.getElementById('load-library').addEventListener('click', () => {
        document.getElementById('load-library-file').click();
    });
    
    document.getElementById('load-library-file').addEventListener('change', loadLibraryFromFile);
    
    // Select all / deselect all buttons
    document.getElementById('select-all-patches').addEventListener('click', () => {
        toggleAllCheckboxes('patches-container', true);
    });
    
    document.getElementById('deselect-all-patches').addEventListener('click', () => {
        toggleAllCheckboxes('patches-container', false);
    });
    
    document.getElementById('select-all-tournaments').addEventListener('click', () => {
        toggleAllCheckboxes('tournaments-container', true);
    });
    
    document.getElementById('deselect-all-tournaments').addEventListener('click', () => {
...
function setupSinglePlayerSelection() {
    ...
    playerDivs.forEach(player => {
        ...
        if (checkedPlayers.includes(player) && !container.querySelector('input:checked')) {
            radio.checked = true;
        }
...
function addNewDeck() {
    ...
    deckItem.querySelector('.add-alias').addEventListener('click', () => addAlias(deckId));
    deckItem.querySelector('.remove-deck').addEventListener('click', () => removeDeck(deckId));
}
...
function removeDeck(deckId) {
    const deckItem = document.querySelector(`.deck-item[data-deck-id="${deckId}"]`);
    if (deckItem) {
        deckItem.remove();
    }
}
...
function displayLoadedLibrary(library) {
    ...
    library.forEach(deck => {
        const deckId = deck.id || `deck-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`;
