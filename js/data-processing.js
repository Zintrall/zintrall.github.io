...
class GameData {
    ...
    async loadData(selectedPatches = [], selectedTournaments = [], startDate = null, endDate = null) {
        this.games = [];
        ...
        for (const patch of selectedPatches) {
            try {
                const patchTournaments = await this.fetchTournamentsForPatch(patch);
                ...
                for (const tournament of patchTournaments) {
                    ...
                    const tournamentData = await this.fetchTournamentData(patch, tournament);
                ...
                console.error(`Error loading data for patch ${patch}:`, error);
            }
        }
    ...
    async fetchTournamentData(patch, tournament) {
        try {
            ...
            if (!response.ok) {
                throw new Error(`Failed to fetch data for tournament ${tournament} in patch ${patch}`);
            }
            ...
            for (const line of lines) {
                const [time, winner, loser, winningHero, losingHero, winningDeck, losingDeck] = line.split('|');
            ...
            console.error(`Error fetching data for tournament ${tournament} in patch ${patch}:`, error);
            return [];
        }
    }
    ...
    getPlayerWinRate(playerName) {
        ...
        const wins = games.filter(game => game.winner.toLowerCase() === playerName.toLowerCase()).length;
    ...
    getHeroWinRates() {
        ...
        for (const hero of this.heroes) {
            const wins = this.games.filter(game => game.winningHero === hero).length;
            const total = this.games.filter(game => 
                game.winningHero === hero || game.losingHero === hero
            ).length;
    ...
    getPlayerHeroStats(playerName) {
        ...
        for (const hero in heroStats) {
            heroStats[hero].winRate = (heroStats[hero].wins / heroStats[hero].played) * 100;
        }
    ...
    getPlayerDeckStats(playerName) {
        ...
        for (const deck in deckStats) {
            deckStats[deck].winRate = (deckStats[deck].wins / deckStats[deck].played) * 100;
        }
        ...
        for (const ownHero in matchups) {
            for (const oppHero in matchups[ownHero]) {
                matchups[ownHero][oppHero].winRate = 
                    (matchups[ownHero][oppHero].wins / matchups[ownHero][oppHero].played) * 100;
            }
        }
    ...
    filterGamesForPlayers(playerNames) {
        ...
        return this.games.filter(game => {
            ...
            return lowerCaseNames.includes(winnerLower) || lowerCaseNames.includes(loserLower);
        });
    }
    ...
    getDeckStatsForPlayers(playerNames, deckLibrary) {
        if (!playerNames || playerNames.length === 0 || !deckLibrary || deckLibrary.length === 0) {
            return [];
        }
        ...
        deckLibrary.forEach(deck => {
            ...
            const aliases = new Set([deck.name.toLowerCase(), ...(deck.aliases || []).map(a => a.toLowerCase())]);
            ...
            selectedGames.forEach(game => {
                ...
                if (aliases.has(winningDeckLower) || aliases.has(losingDeckLower)) {
                    ...
                    if (!deckResult.players[playerName]) {
                        deckResult.players[playerName] = { played: 0, wins: 0, losses: 0, winRate: 0 };
                    }
